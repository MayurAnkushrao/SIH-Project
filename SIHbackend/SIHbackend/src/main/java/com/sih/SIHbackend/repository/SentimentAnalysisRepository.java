...
public interface SentimentAnalysisRepository extends JpaRepository<SentimentAnalysis, Long> {
	...
	// ✅ Method to check if analysis exists for a post by ID
	...
	@Query(value = "SELECT * FROM sentiment_analysis WHERE LOWER(summary) LIKE LOWER(CONCAT('%', :keyword, '%'))", nativeQuery = true)
	List<SentimentAnalysis> findBySummaryContaining(@Param("keyword") String keyword);

	@Query(value = "SELECT * FROM sentiment_analysis WHERE LOWER(keywords) LIKE LOWER(CONCAT('%', :keyword, '%'))", nativeQuery = true)
